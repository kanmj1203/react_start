...
##################### DB에서 id 필드(row) for문으로 배열 호출해서 저장 ######################
...
def get_provider():
    ...
    try:
        for k in soup.find("div", class_="ott_provider").find_all("div"):
            providerName_select = k.find("a")["title"]
            
            if "Netflix" in providerName_select:
                providerName_select = "8"
                str_list.append(providerName_select)
            ...
            providerName_list.append(result)
    except AttributeError as e:
            providerName_select = pymysql.NULL
            providerName_list.append(providerName_select)
#ott_offers_window > section > div.header_poster_wrapper > div > div:nth-child(4) > div > ul > li.ott_filter_best_price.ott_filter_4k > div > a > img
#ott_offers_window > section > div.header_poster_wrapper > div > div:nth-child(4)
#ott_offers_window > section > div.header_poster_wrapper > div > div:nth-child(5)
#ott_offers_window > section > div.header_poster_wrapper > div > div:nth-child(4) > h3
######################################
def get_site():
    soup = BeautifulSoup(html, 'html.parser')
...
for i in range(0,len(movieId)):
    ...
    r = requests.get(f'https://www.themoviedb.org/movie/{j}/watch?translate=true&locale=KR', headers = needed_headers )
    html = r.content
...
#sql = "INSERT INTO movie (cast) VALUES ('%s')" % cast_list
'''
cur.execute("INSERT INTO movie (provider_id) VALUES (%s)", providerName_list)
