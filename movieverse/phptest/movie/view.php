<?php
	...
	$query = $db->query("SELECT * ,COUNT(genres.genres_name) as count , GROUP_CONCAT(genres.genres_name) AS genre_id  from movie left join genres ON movie.genre_id LIKE  CONCAT('%',genres.genre_id,'%')  where id='$id'  group by movie.id");
    if ($row = $query->fetch()) {
...
<h2 style="text-align: center;"> movie 정보 </h2>
