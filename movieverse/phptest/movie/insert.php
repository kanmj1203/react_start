...
<?php
	}else if($db->query("select count(*) from movie where id='$id'")->fetchColumn() > 0) {
?>		
			<script>
				alert('이미 등록된 번호입니다.');
				history.back();
...
<?php
	}else if($db->query("select count(*) from movie where original_title='$original_title'")->fetchColumn() > 0) {
?>		
		<script>
...
<?php
	}else if($db->query("select count(*) from movie where title='$title'")->fetchColumn() > 0) {
?>		
		<script>
			alert('이미 등록된 제목입니다.');
			history.back();
...
<?php
		...
		$query = $db->exec("insert into movie( 
