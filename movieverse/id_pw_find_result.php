...
$_REQUEST["id"] = empty($_REQUEST["id"]) ? Null : $_REQUEST["id"];
$_REQUEST["pw"] = empty($_REQUEST["pw"]) ? Null : $_REQUEST["pw"];
if($_REQUEST["id"]!=Null){
$result ="아이디는 $_REQUEST[id] 입니다";
}else{
$result ="비밀번호는 $_REQUEST[pw] 입니다";
}
?>
...
<form class="login" action="idpw_find.php" method="post">
	...
	<p id="login_text1">로그인</p>
		...
		<input id="id"type="text" name="id" placeholder="<?= $result ?>" readonly>

	<input type="image" name="submit"  class="next"src="img/next.png" border="0">
	<p style="cursor:pointer;"class="find"onclick="location.href='id_pw_find.php';">아이디/비밀번호 찾기</p>
	<p style="top: 476px;cursor:pointer;" class="find"onclick="location.href='member_join.php?bid=';">계정이 없으신가요?가입</p>
	<p class="other">----------------------------------------------------------------------</p>
		<div  style="    top: 266px;   margin-left: 12px;cursor:pointer;" class="google_login"onclick="location.href='login.php';">로그인 화면으로</div>
